...
class MarginCalculation(models.Model):
    ...
    Figma: Margin tab —
      Production Cost, Packaging Cost, Shipping, Marketing,
      Retail Price → Margin % + Profit/unit
    """
    ...
    margin_pct       = models.DecimalField(max_digits=5,  decimal_places=2)  # e.g. 62.0
...
class COGSimulation(models.Model):
    ...
    Figma: COG Sim tab —
      1k units / 5k units / 10k units → cost + breakeven
    """
    id              = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    brand           = models.ForeignKey(
    ...
    # ── Tiers output (JSON) ──
    # e.g. [{"units": 1000, "cost": 12.5, "margin": 62, "breakeven": 450},...]
...
#        Formulation Lab → Saved tab
# ──────────────────────────────────────────────────
class SavedFormulation(models.Model):
    """
    Stores AI-generated formulations from Formulation Lab.
    Figma: Formulation Lab → Generator → Save to Lab
           Saved tab → Ance Serum, formula card
    """
    id              = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    brand           = models.ForeignKey(
    ...
    est_moq         = models.CharField(max_length=50,  blank=True)   # e.g. "2,000 units"
    cost_per_unit   = models.CharField(max_length=50,  blank=True)   # e.g. "£8.90"
    retail_range    = models.CharField(max_length=50,  blank=True)   # e.g. "£38-45"
    key_benefits    = models.JSONField(default=list)
    ph_range        = models.CharField(max_length=20,  blank=True)   # e.g. "5.5 - 6.0"
